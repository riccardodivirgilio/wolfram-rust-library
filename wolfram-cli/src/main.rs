...
fn render_wl(dylib_name: &str, entries: &[FunctionEntry]) -> String {
    ...
    let has_wxf = entries.iter().any(|e| e.kind == "Wxf");
    if has_wxf {
        out.push_str(
    ...
    for e in entries {
        match e.kind.as_str() {
            "Native" => {
                let clean: Vec<String> = e.params.iter().map(|p| p.replace("System`", "")).collect();
                let params = clean.join(", ");
                let ret = e.ret.replace("System`", "");
                out.push_str(&format!(
                    "{} = LibraryFunctionLoad[$lib, \"{}\", {{{}}}, {}];\n",
                    e.name, e.name, params, ret
                ));
            },
            "Wstp" => {
                out.push_str(&format!(
            ...
            "Wxf" => {
                ...
                out.push_str(&format!(
                    "{}Raw = LibraryFunctionLoad[$lib, \"{}\", {}, \
                     {{LibraryDataType[NumericArray, \"UnsignedInteger8\"], Automatic}}];\n",
                    e.name, e.name, arg_spec
                ));
                out.push_str(&format!(
                    "{0}[args___] := fromWxfResult[{0}Raw @@ (toWxfArg /@ {{args}})];\n",
                    e.name
                ));
            },
            other => {
                out.push_str(&format!("(* unknown export kind {other}: {} *)\n", e.name));
            },
        }
    }
